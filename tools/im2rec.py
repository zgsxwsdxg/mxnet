...
def main():
    ...
    if args.list:
        ...
        files = [f for f in os.listdir(args.root)]
        for f in files:
            fpath = os.path.join(os.path.abspath(args.root),f)
            if fpath.endswith('.lst') is True and os.path.isfile(fpath):
                print 'Creating .rec file from', fpath, 'in', args.saving_folder
                image_list = read_list(fpath)
                write_record(args, image_list, fpath)
